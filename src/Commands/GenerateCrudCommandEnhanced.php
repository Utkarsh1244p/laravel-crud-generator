...
namespace UtkarshPG\CrudGenerator\Commands;

use Illuminate\Console\Command;
use Illuminate\Support\Facades\File;
use Illuminate\Support\Str;

class GenerateCrudCommandEnhanced extends Command
{
    ...
    protected function buildSchemaContent($fields)
    {
        ...
        foreach ($fieldDefinitions as $field) {
            ...
            if ($type === 'foreign') {
                ...
                $schemaLines[] = "\$table->foreignId('{$name}')->constrained('{$foreignTable}')->onDelete('cascade');";
            } else {
                $schemaLines[] = "\$table->{$type}('{$name}');";
            }
        }
